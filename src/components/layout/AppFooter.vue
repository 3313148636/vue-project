...
<template>
    <footer>
        <router-link v-for="num in title.length" :key="num" :to="router[num-1]" active-class='active'>
            <span class="nav-icons">
                <svg width="25" height="25" viewBox="0 0 50 50" xmlns="http://www.w3.org/2000/svg"><g fill="#696969" fill-rule="evenodd" v-html="svg[num-1]"></g></svg>
            </span>
            <span>{{title[num-1]}}</span>
        </router-link>
    </footer>
</template>
