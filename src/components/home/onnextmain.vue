
<template>
    <main class="onnextmain" ref="wrapper">
        <ul class="main-ul" v-for='(data,index) in date' :key='index'> 
            <p class="comingTitle">{{data}}</p>
            <li class="main-li" v-for='item in movieList' v-if='item.comingTitle == data' :key='item.id'>
                <div class="left">
                    <img :src="item.img | imgHandle" alt="">
                </div>
                <div class="right">
                    <div class="right-left">
                        <p class="title">
                            <span>{{item.nm}}</span>
                            <span v-if='item.version =="v3d"' class="v3d"></span>
                            <span v-else-if='item.version =="v3d imax"' class="v3d v3d_inmax"></span>
                            <span v-else-if='item.version =="v2d imax"' class="v3d v2d_inmax"></span>
                        </p>
                        <p class="commit" v-if='item.globalReleased'>
                            <span class="span1">观众评</span>
                            <span class="span2"> {{item.sc}}</span>
                        </p>
                        <p class="commit" v-else>
                            <span class="span2"> {{item.wish}}</span>
                            <span class="span1">人想看</span>
                        </p>
                        <p class="actor" v-if='item.star'>
                            主演: {{item.star}}
                        </p>
                        <p class="actor">{{item.showInfo}}</p>
                    </div>
                    <div class="right-right">
                        <p class="buy" v-if='item.globalReleased'>购票</p>
                        <p class="buy2" v-else-if='!item.showInfo'>想看</p>
                        <p class="buy1" v-else>预售</p>
                    </div>
                </div>
            </li>
        </ul>
    </main>
</template>
<script>

export default {
    data(){
        return{
            movieList: [],
            movieIds : [],
            page: 0,
            date: []
        }
    },
    async created(){
        let result = await this.$http({
            url: '/my/ajax/comingList?ci=1&token=&limit=10',
            params: { token:''}
        })
        this.movieIds = result.movieIds;
        this.getItemdata();
    },
    methods: {
       async getItemdata(){
            if(this.page>=25){
                return false;
            }
            let result = await this.$http({
                url: '/my/ajax/moreComingList',
                params: { token:'', movieIds: this.movieIds.splice(0,10).toString() }
            })
            this.page++;
            this.movieList = this.movieList.concat(result.coming);
            var set = new Set();
            for(var i=0; i< this.movieList.length; i++){
                set.add(this.movieList[i].comingTitle)
            }
            this.date = Array.from(set);
        }
    },
    mounted() {
        // this.scroll = scroll({
        //     el: this.$refs.wrapper,
        //     handler: this.getItemdata.bind(this),
        // })

    },
    filters: {
        imgHandle: (val) => {
            if ( !val ) return val;
            return  val.replace(/w.h/g,"128.180");
        }
    }
}
</script> 

<style lang="scss">
    .onnextmain{
        overflow: hidden;
        width: 100%;
        margin-bottom: 1.293333rem;
        height: auto;
        .main-ul{
            padding-left: 15px;
            height: auto;
            .comingTitle{
                padding-top: 12px;
                margin: 0;
                font-size: 14px;
                color: #333;
            }
            .main-li{
                // width: 9.146667rem;
                width: 100%;
                height: 3.04rem;
                overflow: hidden;
                position: relative;
                .left{
                    margin-top: .32rem;
                    width: 1.706667rem;
                    height: 2.4rem;
                    float: left;
                    img{
                        width: 1.706667rem;
                        height: 2.4rem;
                    }
                }
                .right{
                    padding: .32rem 0 .32rem 0;
                    margin-left: 1.973333rem;
                    height: 3.04rem;
                    border-bottom: 1px solid #F6F6F6;
                    .right-left{
                        width: 202px;
                        height: 88px;
                        float: left;
                        .title{
                            max-height: 24px;
                            margin-bottom: 7px;
                            line-height: 24px;
                            overflow: hidden;
                            span:nth-of-type(1){
                                font-size: 17px;
                                color: #333;
                                font-weight: 700;
                            }
                            .v3d{
                                background: url(data:image/png;base64,);
                                background-size: contain;
                                background-repeat: no-repeat;
                                height: 14px;
                                width: 17px;
                                display: inline-block;
                                position: relative;
                                left: 5px;
                            }
                            .v3d_inmax{
                                background: url(data:image/png;base64,);
                                width: 43px;
                                background-size: contain;
                                background-repeat: no-repeat;
                            }
                            .v2d_inmax{
                                background: url(data:image/png;base64,);
                                width: 43px;
                                background-size: contain;
                                background-repeat: no-repeat;
                            }
                        }
                        .commit{
                            color: #666;
                            font-size: 13px;
                            height: .4rem;
                            position: relative;
                            top: -0.053333rem;
                            .span2{
                                font-weight: 700;
                                color: #faaf00;
                                font-size: 13px;
                            }
                        }
                        .actor{
                            font-size: 13px;
                            color: #666;
                            margin-top: 6px;
                            line-height: 15px;
                            text-overflow: ellipsis;
                            overflow: hidden;
                            white-space: nowrap;
                            height: .4rem;
                        }
                    }
                    .right-right{
                        position: absolute;
                        right: .8rem;
                        margin-left: .266667rem;
                        height: 100%;
                        width: 47px;
                        float: left;
                        display: flex;
                        align-items: center;
                        p{
                            height: 27px;
                            line-height: 28px;
                            text-align: center;
                            box-sizing: border-box;
                            background-color: #f03d37;
                            color: #fff;
                            border-radius: 4px;
                            white-space: nowrap;
                            font-size: 12px;
                            cursor: pointer;
                        }
                        .buy1{
                            background-color: #3c9fe6;
                        }
                        .buy2{
                            background-color: #faaf00;
                        }
                    }
                }
            }
        }
    }
</style>
